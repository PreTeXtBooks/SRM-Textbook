...
colnames(OR2Notation) <- c("Group 1", 
                           "Group 2",
                           "Comparing groups")
...
if( knitr::is_latex_output() ) {
  OR2Notation[1, ] <- c( "$n_1$",
                         "$n_2$",
                         NA)
  OR2Notation[2, ] <- c( "$\\text{Odds}_1$", 	
                         "$\\text{Odds}_2$",
                         "$\\text{Odds ratio (OR)} = \\text{Odds}_1/\\text{Odds}_2$")
  OR2Notation[3, ] <- c( "$\\hat{p}_1$",		
                         "$\\hat{p}_2$",
                         "$\\hat{p}_1 - \\hat{p}_2$")
  OR2Notation[4, ] <- c(	"$\\displaystyle\\text{s.e.}(\\hat{p}_1)$",
                         "$\\displaystyle\\text{s.e.}(\\hat{p}_2)$",
                         "$\\displaystyle\\text{s.e.}(\\hat{p}_1 - \\hat{p}_2)$")
  ...
  kable( OR2Notation,
         ...
         longtable = FALSE,
         escape = FALSE,
         linesep = "\\addlinespace", # c("\\addlinespace", "\\addlinespace", "", ""),
...
ENS[3, ] <- c( paste0("\\text{\\llap{OR:\ }$\\phantom{-}",
                      EatingNumericalSummary[3, 1], 
                      "$}"),
               paste0("\\text{\\llap{Difference:\ }$\\phantom{-}", 
                      EatingNumericalSummary[3, 2], 
                      "$}"),
               NA)
...
```{r EatingGraphs, fig.cap="The student-eating data. Left: a side-by-side bar chart. Right: a stacked bar chart.", fig.align="center", fig.height=3.7, fig.width=8.5, out.width="95%"}
par( mfrow = c(1, 2))
...
```{r UniMealsTestExpectedjamovi, fig.cap="The expected counts, as computed by software.", fig.align="center", out.width="70%"}
...
if( knitr::is_latex_output() ) {
  ...
  out <- knitr::kables(list(T1, T2),
                       ...
                       caption = "Identifying rips. Left: the data by age group. Right: a summary table. The ORs are relative to the $51$ to $65$ age group.") %>% 
    kable_styling(font_size = 8)
...
if( knitr::is_html_output() ) {
  knitr::kable( pad(RipSummary,
                ...
                align = "c",
                format = "html",
                caption = "Identifying rips: a summary table. The ORs are relative to the $51$ to $65$ age group.",
                booktabs = TRUE) %>%
    row_spec(0, bold = TRUE) %>%
    add_header_above( c(" " = 1, 
                        "Correctly identifying rips" = 3,
...
if( knitr::is_latex_output() ) {
  rownames(TNsummary) <- c("\\textbf{Natural}",
                         "\\textbf{Relocated}",
                         "")
...
if( knitr::is_html_output() ) {
  kable(pad(TNsummary,
        ...
        longtable = FALSE,
        align = "c",
        caption = "Numerical summary for the turtles data.",
        col.names = c("Odds not infected",
                      "Proportion not infected",
                      "Sample size")) %>%
    row_spec(0, bold = TRUE)
...
```{r TurtleNestsGraphs, fig.show="hold", fig.cap="Bar chart for the turtle-nesting data.", fig.width=4.5, fig.height=3.2, fig.align="center", out.width="50%"}
par(xpd = TRUE,
    mar = c(4, 4, 4, 0.5) + 0.1) # DEFAULT: c(5, 4, 4, 2) + 0.1

if( knitr::is_latex_output()) {
  cols <- grey.colors(n = 2,
                      start = 0.15,
                      end = 0.9)
} else {
  cols <- viridis(9)[c(2, 6)]
}
...
barplot( t(TurtleData), 
         ...
         ylab = "Count",
         xlab = "Type of nest",
         main = "Number of infected and\nnon-infected turtle nests",
         beside = TRUE,
         names.arg = c("Natural", 
                       "Relocated"),
         ylim = c(0, 30),
...
`r if (knitr::is_latex_output()) {
'((Table\\ \\@ref(tab:TurtleNestDataTable), right table) )'
} else {
'(Table\\ \\@ref(tab:TurtleNestSummaryTable))'
}`
shows that the odds of natural nest being infected is $1.66$\ times the odds of a relocated nest being infected.
...
This $z$-score is very small, so expect a large $P$-value.
...
> There is no evidence of a difference in the odds of non-infection ($\chi^2$: $0.777$; $P$-value: $0.378$; OR: $1.657$; $95$%\ CI: $0.537$ to\ $5.12$) between natural nests (odds: $2.90$; $n = 39$) and relocated nests (odds: $1.75$; $n = 22$).
...
if( knitr::is_latex_output() ) {
  ...
  out <- knitr::kables(list(T1, T2),
                       format = "latex",
                       label = "BurrosData",
                       caption = "Left: the health and reproductive status of female burros. Right: row proportions for the burro data (i.e., rows sum to one). Pregnant and lactating burros were counted with the lactating burros only.") %>% 
    kable_styling(font_size = 8)
...
if( knitr::is_html_output() ) {
  kable(pad(BurroSummary,
        ...
        align = "c",
        longtable = FALSE,
        caption = "Row proportions for the burros data (i.e., rows sum to one).",
        booktabs = TRUE)
}
```
...
```{r BurrosChisqjamoviPlot, fig.show="hold", fig.cap="Left: a stacked bar chart for the burro-health data. Right: software output for the burro-health data.", fig.align="center", out.width=c("44%", "55%"), fig.width=c(3, 4), fig.height = 4.25 }
par(mar = c(4, 4, 4, 0.5))
barplot(t(BurroRowProps), 
        ...
        legend.text = c("Excellent", "Fair", "Poor"),
...
`r if( knitr::is_html_output() ) {
mcq(
c(answer = "The odds of having an in-hospital HA is $1.357$ times greater for those with a low body temperature.",
"The odds of having an in-hospital HA is $1.357$ times smaller for those with a low body temperature.",
"The proportion of patients having an in-hospital HA is $1.357$ times greater for those with a low body temperature.",
"The proportion of patients having an in-hospital HA is $1.357$ times smaller for those with a low body temperature.")  )
} else {
c(" 
...
```{r FarmersData}
if( knitr::is_html_output() ) { 
  rownames(Farmers) <- c("Less than $0.5$ ha (small)", 
                       "$\\ge 0.5$ ha (large)")
  kable(pad(Farmers,
            surroundMaths = TRUE,
            targetLength = 2,
...
if( knitr::is_latex_output() ) {
  T2 <- kable(pad(B12Data,
    ...
    add_header_above( c("\\ \\\\null \\ " = 1,
                      ...
                      bold = TRUE,
                      escape = FALSE,
                      line = FALSE) %>% # Causes an error if placed with each table...
    row_spec(0, bold = TRUE) 
...
if( knitr::is_latex_output() ) {
  kable(pad(DogTable,
        ...
        escape = FALSE,
        longtable = FALSE,
        caption = "The length of walks for dogs, living in the city and farms. (`Varies' means usually short walks, but occasional longer walks.)",
        booktabs = TRUE) %>%
    kable_styling(font_size = 8) %>%
    row_spec(0, bold = TRUE) %>%
    column_spec(0, bold = TRUE) %>%
...
and output in
`r if (knitr::is_latex_output()) {
  'Fig.\\ \\@ref(fig:CrabShellsChisqjamovi).'
} else {
...
`r if (knitr::is_latex_output()) {
  '(Fig.\\ \\@ref(fig:CrabShellsChisqjamovi), top output).'
} else {
...
1. Construct a $2\times 2$ table, recording the location of the anemones as either 'Central' or 'Side' without distinguishing *which* side.
 ...
 `r if (knitr::is_latex_output()) {
  '(Fig.\\ \\@ref(fig:CrabShellsChisqjamovi), bottom output).'
} else {
...
```{r CrabShellSetup}
data(CrabShells2) ### Exercise
data(CrabShells3) ### Exercise

CrabTab <- xtabs( Counts ~ ShellRow + ShellColumn,
                  data = CrabShells3)

rownames(CrabTab) <- c("Row: Side 1",
                       "Row: Central",
                       "Row: Side 2")
...
\fontsize{8}{12}\selectfont
```{r}
rownames(CrabTab) <- c("\\textbf{Row}: Side 1",
                       "\\textbf{Row}: Central",
                       "\\textbf{Row}: Side 2")
...
\centering
```{r, out.width='100%'}
knitr::include_graphics( "jamovi/CrabShells/CrabShells3ChisquareTest.png")
knitr::include_graphics( "jamovi/CrabShells/CrabShells2ChisquareTest.png")
```
...
```{r CrabShellsChisqjamoviHTML, fig.cap="Software output for the $3\\times 3$ table of crab-shell data (top output), and for the $2\\times 2$ table of crab-shell data (bottom output).", fig.align="center", out.width="45%", fig.show='hold'}
# The figure for LaTeX is in the minipage (combined with data table), so only need show it for the HTML
knitr::include_graphics( "jamovi/CrabShells/CrabShells3ChisquareTest.png")
knitr::include_graphics( "jamovi/CrabShells/CrabShells2ChisquareTest.png")
```
